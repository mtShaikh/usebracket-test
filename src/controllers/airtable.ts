import { IntersectionFieldType, IFieldMapping } from "./enums.js";
...
export interface AirtableConnection extends Connection {
...
export async function getAllAirtableRecords (
  ...
  if (lookBackPeriod) {
    _.set(
      params,
      "filterByFormula",
      `IS_AFTER(CREATED_TIME(), DATEADD(NOW(), -${lookBackPeriod}, 'h'))`
    );
  }
  ...
  const records = await base(tableId).select(params).all();
...
export async function createAirtableUpdateFieldsGivenFieldMapping  (
  record: any,
  ...
  Object.entries(fieldMapping).forEach(([entry, fieldMappingValue]) => {
    ...
    if (
      ...
      dataType !== IntersectionFieldType.attachment
    ) {
...
export async function createComparableAirtableRecord (
  sourceAirtable: any,
  fieldMapping: IFieldMapping[],
  primaryKey?: string,
  ...
  Object.entries(fieldMapping).forEach(([, fieldMappingValue]) => {
    ...
    switch (fieldType) {
      ...
      case IntersectionFieldType.timestamp: {
        value =
          currentAirtable?.[key] === null ||
          currentAirtable?.[key] === undefined
            ? null
            : new Date(currentAirtable?.[key]);
        _.set(comparableAirtableRecord, newKey, value);
        break;
      }
      case IntersectionFieldType.attachment: {
        ...
        currentAirtable[key]?.forEach((value: any) => {
          if (value && value.url) {
            ...
            console.warn(
              `value was not present for attachment array: ${value}`
            );
          }
        });

        _.set(
          comparableAirtableRecord,
          newKey,
          outputArray.length === 0 ? null : outputArray
        );
        break;
      }
...
export async function mutateAirtableRecord(
  data: { id?: any },
  airtableAction: AirtableAction,
  airtableRecordId: Optional<string>,
  baseId: string,
  tableName: string,
  apiKey: string
): Promise<string> {
  ...
  switch (airtableAction) {
    case AirtableAction.CREATE: {
      const createResponse = await base(tableName)
        .create(data, { typecast: true })
        .catch((e) => {
          utilPrint({ e });
          throw e
        });
    ...
    case AirtableAction.UPDATE: {
      const updateResponse = await base(tableName)
        .update(
          airtableRecordId.orElseThrow(() => {
...
export async function pushToAirtableForGivenFieldMapping(
  ...
  let maybeAirtableRecordId: Optional<string> =
    Optional.ofNullable(airtableRecordId);
  if (maybeAirtableRecordId.isPresent()) {
    maybeAirtableRecordId = Optional.ofNullable(
      await mutateAirtableRecord(
        payloadToAirtable,
        AirtableAction.UPDATE,
        maybeAirtableRecordId,
        baseId,
        tableId,
        apiKey
      )
